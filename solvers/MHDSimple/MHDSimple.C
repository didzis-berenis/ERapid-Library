...
int main(int argc, char *argv[])
{
    ...
    double maxErr = 1;
    double timeOF = 0;
    double timeElmer = runTime.elapsedClockTime();
    ...
    timeElmer = runTime.elapsedClockTime() - timeElmer;

    while (simple.loop())
    {
        Info<< "Time = " << runTime.timeName() << nl << endl;
        timeOF = runTime.elapsedClockTime();
        ...
        timeOF = runTime.elapsedClockTime() - timeOF;
	maxErr = (max(mag(U.oldTime()-U))/max(mag(U))).value();

        Info<< "ExecutionTime = " << runTime.elapsedCpuTime() << " s"
            << "  ClockTime = " << runTime.elapsedClockTime() << " s"
            << "  OF step = " << timeOF << " s"
            << "  Elmer step = " << timeElmer << " s"
            << "  err = ( " << maxErr << " )" << nl << endl;
        timeElmer = runTime.elapsedClockTime();
